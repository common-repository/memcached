...
= 4.0.1 =
* Add local caching when an add() fails. This will reduce the things which break when Memcached is not behaving as expected. For example - failing to add `alloptions` to the local cache will hammer the database which on large options in particular can cause problems.
